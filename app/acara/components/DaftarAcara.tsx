...
const TipeBadge = memo(({ tipe }: { tipe: string }) => {
  ...
  return (
    <span
      className={`px-2 py-0.5 text-xs rounded-full ${colors[tipe as keyof typeof colors] || colors.umum}`}
    >
      {tipe}
    </span>
  )
})
TipeBadge.displayName = 'TipeBadge'
              ...
              <div className="flex-1">
                ...
                <div className="space-y-1 text-sm text-slate-600">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      {format(new Date(item.tanggal_mulai), 'EEEE, dd MMM yyyy HH:mm', {
                        locale: id,
                      })}{' '}
                  ...
                  {item.deskripsi && (
                    <p className="mt-2 text-slate-500 line-clamp-2">{item.deskripsi}</p>
                  )}
                </div>
              </div>
