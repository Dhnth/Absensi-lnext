...
export default function KelasPage() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Yakin ingin menghapus kelas ini? Anggota dengan kelas ini akan menjadi null.'))
      return
    ...
    await supabase.from('anggota').update({ kelas: null }).eq('kelas', kelasYangDihapus?.nama)
  ...
  return (
    <div className="space-y-6 p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          <div>
            ...
            <p className="text-slate-600 mt-1">Kelola data kelas untuk dropdown anggota</p>
          </div>
        </div>
      ...
      {/* Tabel Kelas */}
      <TabelKelas data={data} loading={loading} onEdit={handleEdit} onDelete={handleDelete} />
