...
export default function DaftarAbsensiPage() {
    ...
    try {
      ...
      if (status === 'alpha') {
        ...
        if (selectedRow.absensi) {
          await supabase.from('absensi').delete().eq('id', selectedRow.absensi.id)
        }
      } else {
        ...
        if (selectedRow.absensi) {
          // UPDATE
          await supabase.from('absensi').update(absensiData).eq('id', selectedRow.absensi.id)
        } else {
          // INSERT
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      {/* Info */}
      <div className="text-sm text-slate-500">Menampilkan {filteredData.length} anggota</div>
